...
export const getJobsByUser = asyncHandler(async (req,res) => {
  try {
    ...
    const jobs = await Job.find({createdBy: user._id}).populate("createdBy",
      "name profilePicture"
    );
    return res.status(200).json(jobs);
  } catch (error) {
...
export const searchJobs = asyncHandler(async (req,res) => {
  try {
    ...
    if (location) {
      query.location = { $regex: location, $option: "i" };
    }
    ...
    const jobs  = await Job.find(query).populate("createdBy", "name proPicture");
